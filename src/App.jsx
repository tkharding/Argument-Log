        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:6 }}>
          ...
          {!addingTopic && (
            <button onClick={() => setAddingTopic(true)} style={{
              padding:"5px 12px", borderRadius:20,
              border:"1.5px dashed #b8d4c4", background:"transparent",
      ...
      {toast && (
        <div style={{ marginBottom:16, animation:"fadeInOut 10s ease forwards" }}>
          <style>{`
            ...
            .thought-cloud::before { width:36px; height:36px; bottom:-28px; left:36px; }
            .thought-cloud::after  { width:22px; height:22px; bottom:-50px; left:22px; }
          `}</style>
          <div className="thought-cloud" style={{
            '--cloud-bg': toast.color+'22', '--cloud-border': toast.color+'88',
            '--cloud-shadow': toast.color+'33',
            fontSize:16, color:"#1a3040", lineHeight:1.7,
            fontStyle:"italic", textAlign:"center", fontWeight:700,
          }}>
            {toast.message}
            <div style={{ position:"absolute", bottom:"-68px", left:"12px",
              width:14, height:14, borderRadius:"50%",
              background:toast.color+"22", border:`2px solid ${toast.color}88` }}/>
          </div>
        </div>
      )}
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                  ...
                  <div style={{ display:"flex", gap:4, marginLeft:8, flexShrink:0 }}>
                    <button onClick={() => setEditing(e.id)}
                      style={{ background:"none",border:"none",color:"#3a5a6a",fontSize:15,cursor:"pointer",padding:"2px 5px" }}>✎</button>
...
function SummaryStats({ data, year, month }){
  ...
  const allEntries = Object.entries(data).flatMap(([k,entries]) => entries.map(e => ({ ...e, dateKey:k })));
  const monthPrefix = `${year}-${String(month+1).padStart(2,"0")}`;
  const scoped = scope==="month" ? allEntries.filter(e => e.dateKey.startsWith(monthPrefix)) : allEntries;
  ...
  const yearlyByInt = {};
  if(useYearly) allEntries.forEach(e => {
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
      ...
      <div>
        ...
        <div style={{ display:"flex", gap:6 }}>
          {INTENSITIES.map((it,i) => {
            const c=intCounts[i], pct=total>0?Math.round((c/total)*100):0;
            return (
        ...
        <div>
          ...
          <div style={{ background:"#e8f0ec", borderRadius:12, padding:"16px 8px 8px" }}>
            {(()=>{
              ...
              const iW=W-padL-padR,iH=H-padT-padB,n=trendKeys.length;
              const xOf=i=>padL+(n===1?iW/2:(i/(n-1))*iW);
              const yOf=v=>padT+iH-(v/maxTrendVal)*iH;
              const gv=[0,Math.round(maxTrendVal/2),maxTrendVal];
              const cp=`${year}-${String(month+1).padStart(2,"0")}`;
              return (
                <svg viewBox={`0 0 ${W} ${H}`} style={{ width:"100%", height:"auto", overflow:"visible" }}>
                  ...
                  {trendSeries.map((vals,si)=>{
                    ...
                    return <polyline key={si} points={vals.map((v,i)=>`${xOf(i)},${yOf(v)}`).join(" ")}
                      fill="none" stroke={color} strokeWidth="2" strokeLinejoin="round" strokeLinecap="round"/>;
                  })}
                  {trendSeries.map((vals,si)=>vals.map((v,i)=>{
                    const color=INTENSITIES[si].color,cx=xOf(i),cy=yOf(v),isCur=trendKeys[i]===cp;
                  ...
                  {trendKeys.map((k,i)=>{
                    ...
                    const isCur=useYearly?k===String(year):k===cp;
    ...
    <div style={{ minHeight:"100vh", background:"#f0f4f8", display:"flex",
      ...
      <div style={{ textAlign:"center", marginBottom:32 }}>
        <div style={{ position:"relative", display:"inline-block", marginBottom:12 }}>
          <style>{`
            .logo-cloud { background:#1a237e18; border:2.5px solid #1a237e99;
              border-radius:50% 50% 50% 50%/40% 40% 60% 60%;
              padding:14px 32px; position:relative; display:inline-block; }
            .logo-cloud::before { content:''; position:absolute; width:22px; height:22px;
              background:#1a237e18; border:2.5px solid #1a237e99; border-radius:50%;
              bottom:-18px; left:28px; }
            .logo-cloud::after { content:''; position:absolute; width:13px; height:13px;
              background:#1a237e18; border:2px solid #1a237e99; border-radius:50%;
...
function FolderSetupScreen({ onSave, existingFolder, onCancel, token }){
  ...
  async function handleSave(){
    ...
    setLoading(true);
    const fn = folderName||`${name1} & ${name2}`;
    const res = await apiFetch("/api/folder","POST",{folderName:fn,name1,name2},token);
    if(res.error){ setError(res.error); setLoading(false); return; }
    onSave({ id:res.folder.id, folderName:fn, name1, name2 });
    setLoading(false);
  }
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div>
            ...
            <input value={folderName} onChange={e=>setFolderName(e.target.value)}
              placeholder={name1&&name2?`${name1} & ${name2}`:"e.g. Home Life"} style={inputStyle} />
          </div>
          {error && (
            <div style={{ fontSize:13, color:"#b04a4a", background:"#b04a4a18",
              border:"1px solid #b04a4a44", borderRadius:8, padding:"8px 12px" }}>{error}</div>
          ...
          <button onClick={handleSave} disabled={loading} style={{
            ...solidBtn("#1a6b3a"), width:"100%", padding:"13px", fontSize:16, marginTop:4, opacity:loading?0.7:1,
          }}>
            {loading?"Saving...":existingFolder?"Save Changes":"Get Started"}
          </button>
          {onCancel && (
...
function SettingsPage({ user, folder, onEditFolder, onDeleteData, onLogout, onClose, token }){
  const [confirmDelete, setConfirmDelete] = useState(false);
  const [deleting,      setDeleting]      = useState(false);

  async function handleDelete(){
    setDeleting(true);
    await apiFetch("/api/entries/all","DELETE",null,token);
    onDeleteData();
    setDeleting(false);
  }
    ...
    <div style={{ minHeight:"100vh", background:"#f0f4f8", display:"flex",
      ...
      <div style={{ width:"100%", maxWidth:400 }}>
        ...
        <div style={{ background:"#ffffff", border:"1px solid #b8d4c4", borderRadius:16, padding:"20px", marginBottom:16 }}>
          ...
          {folder && <div style={{ fontSize:13, color:"#3a5a6a", marginTop:2 }}>{folder.name1} & {folder.name2}</div>}
          <button onClick={onEditFolder} style={{ ...solidBtn("#1a237e"), marginTop:14, width:"100%", padding:"11px" }}>
            Edit Names / Folder
          </button>
        </div>
            ...
            <div style={{ marginTop:14 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                ...
                <button onClick={handleDelete} disabled={deleting}
                  style={{ ...solidBtn("#c0392b"), flex:1, padding:"11px" }}>
...
export default function App(){
  ...
  const [loading,  setLoading]  = useState(false);
  const [token,    setToken]    = useState(() => localStorage.getItem("conflict_log_token")||null);
  const [user,     setUser]     = useState(() => {
    try { return JSON.parse(localStorage.getItem("conflict_log_user")); } catch { return null; }
  });
  ...
  function handleLogout(){
    localStorage.removeItem("conflict_log_token");
    localStorage.removeItem("conflict_log_user");
    setUser(null); setToken(null); setData({}); setFolder(null); setCustomTopics([]);
    setScreen("app");
  }

  function handleDeleteData(){ setData({}); setScreen("app"); }

  async function addCustomTopic(t){
    setCustomTopics(prev => prev.includes(t)?prev:[...prev,t]);
    await apiFetch("/api/topics","POST",{topic:t},token);
  }

  async function addEntry(dateKey,entry){
    const res = await apiFetch("/api/entries","POST",{dateKey,entry,folderId:folder?.id},token);
    const cloudId = res.id||entry.id;
    setData(d=>({...d,[dateKey]:[...(d[dateKey]||[]),{...entry,id:cloudId}]}));
  }

  async function updateEntry(dateKey,entry){
    await apiFetch(`/api/entries/${entry.id}`,"PUT",entry,token);
    setData(d=>({...d,[dateKey]:d[dateKey].map(e=>e.id===entry.id?entry:e)}));
  }

  async function deleteEntry(dateKey,id){
    await apiFetch(`/api/entries/${id}`,"DELETE",null,token);
    setData(d=>{
  ...
  function handleExport(){
    const allEntries=Object.entries(data).flatMap(([k,entries])=>entries.map(e=>({...e,dateKey:k}))).sort((a,b)=>a.dateKey.localeCompare(b.dateKey));
    const lines=["ARGUMENT LOG EXPORT","===================",""];
    let lastMonth="";
    allEntries.forEach(e=>{
      const mo=e.dateKey.slice(0,7);
      if(mo!==lastMonth){ lines.push(`\n--- ${MONTHS[parseInt(mo.split("-")[1])-1]} ${mo.split("-")[0]} ---`); lastMonth=mo; }
  ...
  if(loading) return (
    <div style={{ minHeight:"100vh", background:"#f0f4f8", display:"flex",
      ...
      <div style={{ textAlign:"center" }}>
        <div style={{ fontSize:18, color:"#2a4a5a", fontWeight:600 }}>Loading your data...</div>
        <div style={{ fontSize:14, color:"#3a5a6a", marginTop:8 }}>Syncing from the cloud ☁️</div>
      </div>
    </div>
  );

  if(screen==="folderSetup") return (
    <FolderSetupScreen existingFolder={folder} onSave={f=>{setFolder(f);setScreen("app");}}
      onCancel={folder?()=>setScreen("app"):null} token={token} />
  );

  if(screen==="settings") return (
    <SettingsPage user={user} folder={folder}
      onEditFolder={()=>setScreen("folderSetup")}
      onDeleteData={handleDeleteData} onLogout={handleLogout}
      onClose={()=>setScreen("app")} token={token} />
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#f0f4f8", display:"flex",
      flexDirection:"column", alignItems:"center", padding:"28px 16px 48px",
      fontFamily:"'Palatino Linotype','Book Antiqua',Palatino,serif" }}>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.4)",
          ...
          <div style={{ background:"#ffffff", borderRadius:16, width:"100%", maxWidth:400,
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <button onClick={()=>setExportText(null)} style={{ background:"none",border:"none",fontSize:20,color:"#3a5a6a",cursor:"pointer" }}>✕</button>
            </div>
            ...
            <textarea readOnly value={exportText} onClick={e=>e.target.select()}
              style={{ flex:1, minHeight:280, background:"#e8f0ec", border:"1.5px solid #b8d4c4",
                borderRadius:8, color:"#1a3040", fontSize:12, padding:"10px",
                fontFamily:"monospace", resize:"none", outline:"none" }} />
            <button onClick={()=>{ navigator.clipboard?.writeText(exportText).then(()=>alert("Copied!")).catch(()=>alert("Please manually select and copy.")); }}
      ...
      <div style={{ textAlign:"center", marginBottom:24, width:"100%", maxWidth:400 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          <button onClick={handleExport} style={{ background:"none", border:"1px solid #b8d4c4",
            borderRadius:8, color:"#3a5a6a", fontSize:12, padding:"5px 10px", cursor:"pointer" }}>Export</button>
          <div style={{ textAlign:"center" }}>
        ...
        {folder && <div style={{ fontSize:14, color:"#3a5a6a", fontWeight:600 }}>{folder.name1} & {folder.name2}</div>}
      </div>

      <div style={{ display:"flex", marginBottom:18, background:"#ffffff", border:"1px solid #b8d4c4",
        borderRadius:12, overflow:"hidden", width:"100%", maxWidth:400, padding:4, gap:4 }}>
      ...
      {tab==="stats" && (
        <div style={{ background:"#ffffff", border:"1px solid #b8d4c4", borderRadius:16,
          width:"100%", maxWidth:400, padding:"22px 18px", boxShadow:"0 8px 24px rgba(0,0,0,0.08)" }}>
      ...
      {tab==="calendar" && (
        <>
          <div style={{ background:"#ffffff", border:"1px solid #b8d4c4", borderRadius:16,
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3 }}>
              {cells.map((day,i)=>{
                ...
                const key=toKey(year,month,day), color=dotColor(key);
                const isSel=selected===day, tod=isToday(day);
                return (
                  <button key={day} onClick={()=>setSelected(isSel?null:day)} style={{
                    ...
                    {color && <span style={{ display:"block", width:5, height:5, borderRadius:"50%",
          ...
          {selected && (
            <DayPanel dateKey={selectedKey} dateLabel={selectedLabel} entries={selectedEntries}
              onAdd={addEntry} onUpdate={updateEntry} onDelete={deleteEntry}
              customTopics={customTopics} onAddCustomTopic={addCustomTopic} />
          )}
        </>
      )}
    </div>
